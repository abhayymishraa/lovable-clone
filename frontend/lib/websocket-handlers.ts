...
export function handleWebSocketMessage(
  ...
  try {
    ...
    if (data.e === 'tool_completed') {
      const toolName = (data.tool_name as string) || handlers.currentTool?.name || 'Tool';
            ...
            return {
              ...
              tool_calls: msg.tool_calls.map(tool => 
                tool.name === toolName && tool.status === 'running'
                  ? { ...tool, status: 'success' as const, output: typeof toolOutput === 'string' ? toolOutput : JSON.stringify(toolOutput) }
                  : tool
              )
            };
          }
    ...
    if (data.type === 'history' && data.messages) {
      ...
      console.log('📦 Consolidated to:', consolidatedMessages.length, 'messages');
      handlers.setMessages(consolidatedMessages);
      
      // Set app URL if it exists - skip health check for restored URLs
      if (data.app_url) {
        console.log('✅ Setting app URL directly (no health check for restored URL):', data.app_url);
    ...
    if (data.e === 'thinking' && data.message) {
      handlers.setMessages(prev => {
        ...
        const looksLikeJson = newContent.trim().startsWith('{') || 
